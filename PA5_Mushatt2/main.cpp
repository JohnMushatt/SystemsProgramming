...
void runSimulation(float simulationTime, EventQueue* eventQueue,
		CustomerEvent** customersCollection, int customers,
		TellerEvent** tellersCollection, int tellers, TellerQueue** tellerLines,
		bool shareLines);
	...
	for (int i = 0; i < 2; i++) { // RUN PROGRAM TWICE
		...
		if (i == 0) { // First Run has teller with seperate lines
			...
			for (int i = 0; i < tellers; i++) {
				...
				float serviceTime = 2 * averageServiceTime * rand()
						/ float(RAND_MAX);
				...
				// Create Teller
				TellerEvent* newTeller = new TellerEvent(serviceTime, idleTime,
						tellerLines[i]);
			...
			runSimulation(simulationTime, eventQueue, customersCollection,
					customers, tellersCollection, tellers, tellerLines, false);
			...
			runSimulation(simulationTime, eventQueue, customersCollection,
					customers, tellersCollection, tellers, onlyTellerLine,
					true);
		}
	...
	for (int i = 0; i < numCustomers; i++) {
		if (customers[i]->getServicing()) {
			if (customers[i]->getLeaveTime() > 0) { // Did the customer leave?
				numServed++;
				timeSpent = abs(
						customers[i]->getLeaveTime()
								- customers[i]->getArrivalTime());
				sum += timeSpent;
			}
		}
	}
...
float findMaxWait(CustomerEvent** customers, int numCustomers) {
	...
	for (int i = 1; i < numCustomers; i++) {
		if (customers[i]->getLeaveTime() > 0) { // Did the customer leave?
			timeSpent = customers[i]->getLeaveTime()
					- customers[i]->getArrivalTime();
			if (timeSpent > max) {
				max = timeSpent;
			}
...
//			printEQ(eventQueue);
			...
			if (shareLines) {
				...
				eventQueue->top()->action(CLOCK, tellerLines, tellers,
						eventQueue);
			}
		} else {
			break;
	...
	float totalTellerService = findTotalServiceTimes(tellersCollection,
			tellers);
